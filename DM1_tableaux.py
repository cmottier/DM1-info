...
B = np.array(
    [
        [27.8, 189.7, 70, 119.6, 187.1, 76.9],
...
for j in range(5):
    for i in range(6):
        ICAS += Proba[i, j] * log(
            Proba[i, j]
            / ((np.sum(Proba[i, :]) + np.sum(Proba[i + 6, :])) * np.sum(Proba[0:6, j]))
        )
    for i in range(6, 12):
        ICAS += Proba[i, j] * log(
            Proba[i, j]
            / ((np.sum(Proba[i, :]) + np.sum(Proba[i - 6, :])) * np.sum(Proba[6:12, j]))
        )
...
for j in range(5):
    for i in range(6):
        ISAC += Proba[i, j] * log(
            Proba[i, j] / (np.sum(Proba[0:6, :]) * (Proba[i, j] + Proba[i + 6, j]))
        )
    for i in range(6, 12):
        ISAC += Proba[i, j] * log(
            Proba[i, j] / (np.sum(Proba[6:12, :]) * (Proba[i, j] + Proba[i - 6, j]))
        )
...
for j in range(5):
    for i in range(12):
        IACS += Proba[i, j] * log(
            Proba[i, j] / (np.sum(Proba[:, j]) * np.sum(Proba[i, :]))
        )
...
IAS = 0
for j in range(5):
    IAS += np.sum(Proba[0:6, j]) * log(
        np.sum(Proba[0:6, j]) / (np.sum(Proba[:, j]) * np.sum(Proba[0:6, :]))
    )
    IAS += np.sum(Proba[6:12, j]) * log(
        np.sum(Proba[6:12, j]) / (np.sum(Proba[:, j]) * np.sum(Proba[6:12, :]))
    )
...
for j in range(5):
    for i in range(6):
        IAC += (Proba[i, j] + Proba[i + 6, j]) * log(
            (Proba[i, j] + Proba[i + 6, j])
            / (np.sum(Proba[:, j]) * (np.sum(Proba[i, :]) + np.sum(Proba[i + 6, :])))
        )
...
ISC = 0
for i in range(6):
    ISC += np.sum(Proba[i, :]) * log(
        np.sum(Proba[i, :])
        / (np.sum(Proba[0:6, :]) * (np.sum(Proba[i, :]) + np.sum(Proba[i + 6, :])))
    )
for i in range(6, 12):
    ISC += np.sum(Proba[i, :]) * log(
        np.sum(Proba[i, :])
        / (np.sum(Proba[6:12, :]) * (np.sum(Proba[i, :]) + np.sum(Proba[i - 6, :])))
    )

print("IAS", round(IAS, 7), "IAC", IAC, "ISC", ISC)
print("I(C) = ", IAC + ISC, "I(A) = ", IAC + IAS, "I(S) = ", IAS + ISC)
...
def IXY(j):
    return (
        np.sum(XY[0, 0:j])
        * log(np.sum(XY[0, 0:j]) / (np.sum(XY[0, :]) * np.sum(XY[:, 0:j])))
        + np.sum(XY[1, 0:j])
        * log(np.sum(XY[1, 0:j]) / (np.sum(XY[1, :]) * np.sum(XY[:, 0:j])))
        + np.sum(XY[0, j:])
        * log(np.sum(XY[0, j:]) / (np.sum(XY[0, :]) * np.sum(XY[:, j:])))
        + np.sum(XY[1, j:])
        * log(np.sum(XY[1, j:]) / (np.sum(XY[1, :]) * np.sum(XY[:, j:])))
    )
